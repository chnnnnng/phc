...
{
    gpio_init(ABS_ENCODER_SCK_PIN , GPO,1);
    gpio_init(ABS_ENCODER_MOSI_PIN, GPO,0);
    gpio_init(ABS_ENCODER_CSN_PIN , GPO,1);
    gpio_init(ABS_ENCODER_MISO_PIN ,GPI,0);
...
{  
	spi_mosi(ABS_ENCODER_SPI_NUM, SPI_CS_NOT_USE, &byte,&byte,1);
	return(byte);   
}
...
void encoder_init_spi(void)
{
    ...
    gpio_init(ABS_ENCODER_CSN_PIN , GPO,1);
	//初始化SPI，使用软件控制CS引脚
	(void)spi_init(ABS_ENCODER_SPI_NUM,SPI_CS_NOT_USE,MASTER,10*1000*1000);
